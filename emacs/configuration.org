...
#+RESULTS:
| ~/orgs/gtd.org | ~/orgs/code.org | ~/orgs/journal.org |
...
#+RESULTS:
| (lambda nil (let ((filename (buffer-file-name (current-buffer)))) (when (and filename (string= trello (file-name-extension filename))) (org-trello-mode)))) | #[nil \301\302"\205	 \303 \207 [org-trello-files -any? #[(name) \302!	\230\207 [name buffer-file-name expand-file-name] 2] org-trello-mode] 3] | (lambda nil (let ((filename (buffer-file-name (current-buffer)))) (if (and filename (string= trello (file-name-extension filename))) (progn (org-trello-mode))))) | org-pdftools-setup-link | org-ref-org-menu | #[0 \300\301\302\303\304$\207 [add-hook change-major-mode-hook org-show-all append local] 5] | #[0 \300\301\302\303\304$\207 [add-hook change-major-mode-hook org-babel-show-result-all append local] 5] | org-babel-result-hide-spec | org-babel-hide-all-hashes | #[0 \301\211\207 [imenu-create-index-function org-imenu-get-tree] 2] | org-indent-mode | (lambda nil (org-bullets-mode 1)) | org-ref-setup-label-finders |
